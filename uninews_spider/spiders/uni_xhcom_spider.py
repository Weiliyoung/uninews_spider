...
from uninews_spider.items.uni_xhcom import XhcomItem


class XHCOMSpider(scrapy.Spider):
    name = 'xhcom_spider'
    allowed_domains = ['yjsb.xhcom.edu.cn']
    start_urls = ['https://yjsb.xhcom.edu.cn/']
    custom_settings = {
    ...
    def parse(self, response):
        ...
        # 在此处添加提取招生就业链接的代码
        recruitment_url = response.xpath('//div[@class="nav"]//div/ul/li[5]/a/@href').get()
        if recruitment_url:
    ...
    def parse_news_list(self, response):
        # 提取所有新闻条目的链接
        news_links = response.xpath('//div[@class="vsb-box"]//div[2]/ul/li/a/@href').getall()
        self.logger.info(f"当前页面 {response.url} 包含的所有的url: {news_links}")
        for link in news_links:
            yield response.follow(link, callback=self.parse_news_content)

        # 提取下一页的链接并递归跟踪
        next_page_link = response.xpath('//div[@class="vsb-box"]//div//tbody/tr/td//tbody/tr/td[2]/div/a[1]/@href').get()
        if next_page_link:
    ...
    def parse_news_content(self, response):
        ...
        # 提取内容，合并所有段落
        content = ''.join(response.xpath('//div[@class="vsb-box"]//div/p/span/text()').getall()).strip()
