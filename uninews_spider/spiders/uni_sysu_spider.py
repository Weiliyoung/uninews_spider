...
class SYSUSpider(scrapy.Spider):
    ...
    def parse_recruitment_list(self, response):
        ...
        # 在此添加提取硕士招生所有公告链接
        news_links = response.xpath('//*[@class="views-element-container"]//ul/li/a/@href').getall()
        self.logger.info(f"当前页面 {response.url} 包含的所有的url: {news_links}")
        for link in news_links:
            yield response.follow(link, callback=self.parse_news_content)

        # 提取下一页的链接并递归跟踪
        next_page_link = response.xpath('//*[@class="pager outside-tb t-c"]/ul/li/a/@href').get()
        if next_page_link:
    ...
    def parse_news_content(self, response):
        # 提取标题
        title = response.xpath('//*[@id="content"]//h1/text()').get().strip()
        # 提取来源
        source = response.xpath('//*[@id="content"]//div[contains(@class, "article-submit inside-min-tb")]/span[1]/text()')
        # 提取时间
        date = response.xpath('//*[@id="content"]//div[contains(@class, "article-submit inside-min-tb")]/span[3]/text()').get().strip()
        # 提取内容
        content = ''.json(response.xpath('//*[@id="content"]//div[contains(@class, "field")]/p//text()')).getall().strip()
        # 页面URL
        url = response.url
        # 爬虫时间
