...
from uninews_spider.items.uni_fosu import FosuItem


class FOSUSpider(scrapy.Spider):
    name = 'fosu_spider'
    allowed_domains = ['fosu.edu.cn']
    start_urls = ['https://www.fosu.edu.cn/yanjiusheng']
    custom_settings = {
    ...
    def parse(self, response):
        ...
        # 在此处添加提取招生就业链接的代码
        recruitment_url = response.xpath('//div[3]/div[3]/div[1]/div[3]//div/span/a/@href').get()
        if recruitment_url:
    ...
    def parse_news_list(self, response):
        # 提取所有新闻条目的链接
        news_links = response.xpath('//div[@class="category1_trap"]//div/ul/li/a/@href').getall()
        self.logger.info(f"当前页面 {response.url} 包含的所有的url: {news_links}")
        for link in news_links:
            yield response.follow(link, callback=self.parse_news_content)

        # 提取下一页的链接并递归跟踪
        next_page_link = response.xpath('//div[@class="page"]//div/center/a[11]/@href').get()
        if next_page_link:
    ...
    def parse_news_content(self, response):
        ...
        # 提取标题
        title = response.xpath('//div[@class="articleTitle"]/h2/text()').get().strip()
        ...
        # 提取时间
        date = response.xpath('//div[@class="articleAuthor"]/span[1]/text()').get().strip()

        # 提取内容，合并所有段落
        content = ''.join(response.xpath('//div[@class="article"]/p/span/text()').getall()).strip()
