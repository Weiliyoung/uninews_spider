...
from uninews_spider.items.uni_zqu import ZquItem


class ZQUSpider(scrapy.Spider):
    name = 'zqu_spider'
    allowed_domains = ['yjsc.zqu.edu.cn']
    start_urls = ['https://yjsc.zqu.edu.cn/']
    custom_settings = {
    ...
    def parse(self, response):
        ...
        # 在此处添加提取招生就业链接的代码
        recruitment_url = response.xpath('//*[@id="a_0_1004"]/a/@href').get()
        if recruitment_url:
    ...
    def parse_news_list(self, response):
        # 提取所有新闻条目的链接
        news_links = response.xpath('//table[2]//tr[3]/td[2]//tr[1]/td[1]/a/@href').getall()
        self.logger.info(f"当前页面 {response.url} 包含的所有的url: {news_links}")
        for link in news_links:
            yield response.follow(link, callback=self.parse_news_content)

        # 提取下一页的链接并递归跟踪
        next_page_link = response.xpath('//table[2]//tr[3]/td[2]//tr[12]/td//tr/td//tr/td[2]/div/a[4]/@href').get()
        if next_page_link:
    ...
    def parse_news_content(self, response):
        ...
        # 提取标题
        title = response.xpath('//tr[3]/td[2]/form/table/tbody/tr[1]/td/text()').get().strip()
        ...
        # 提取时间
        date = response.xpath('//tr[3]/td[2]/form//tr[2]/td/span[1]/text()').get().strip()
