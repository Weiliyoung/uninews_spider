...
from uninews_spider.items.uni_dgut import DgutItem


class DGUTSpider(scrapy.Spider):
    name = 'dgut_spider'
    allowed_domains = ['yjs.dgut.edu.cn']
    start_urls = ['https://yjs.dgut.edu.cn/']
    custom_settings = {
    ...
    def parse(self, response):
        ...
        # 在此处添加提取招生就业链接的代码
        recruitment_url = response.xpath('//a[text()="招生信息"]/@href').get()
        if recruitment_url:
    ...
    def parse_news_list(self, response):
        ...
        # 提取下一页的链接并递归跟踪
        next_page_link = response.xpath('//div[4]//div/span/span[10]/a/@href').get()
        if next_page_link:
    ...
    def parse_news_content(self, response):
        ...
        # 提取时间
        date = response.xpath('//div[3]/div[2]/form/div/div[1]/p/text()').get().strip()
