...
class GZUMpider(scrapy.Spider):
    ...
    start_urls = ['https://yjsxy.gdmu.edu.cn/']
    custom_settings = {
    ...
    def parse(self, response):
        ...
        # 在此处添加提取硕士招生链接的代码
        recruitment_url = response.xpath('//*[@id="a_0_1006"]/a/@href').get()
        if recruitment_url:
    ...
    def parse_recruitment_list(self, response):
        ...
        # 提取下一页的链接并递归跟踪
        next_page_link = response.xpath('//table[4]//tr[2]//div//tr[12]//td[2]/div/a[1]/@href').get()
        if next_page_link:
