...
from uninews_spider.items.uni_hitsz import HitszItem


class HITSZSpider(scrapy.Spider):
    name = 'hitsz_spider'
    allowed_domains = ['yzb.hitsz.edu.cn']
    start_urls = ['https://yzb.hitsz.edu.cn/zs']
    custom_settings = {
    ...
    def parse(self, response):
        ...
        # 在此处添加提取硕士公告链接的代码
        recruitment_url = response.xpath('//div[@class="container_box"]/div/div[1]/div/a/@href').get()
        if recruitment_url:
    ...
    def parse_recruitment_list(self, response):
        self.logger.debug("Parsing started for URL:%s", response.url)
        # 在此添加提取硕士招生所有公告链接
        news_links = response.xpath('//div[@class="second_list"]//div/a/@href').getall()
        self.logger.info(f"当前页面 {response.url} 包含的所有的url: {news_links}")
        for link in news_links:
            yield response.follow(link, callback=self.parse_news_content)

        # 提取下一页的链接并递归跟踪
        next_page_link = response.xpath('//a[@class="k-link k-pager-nav"]/@href').get()
        if next_page_link:
    ...
    def parse_news_content(self, response):
        # 提取标题
        title = response.xpath('//div[@class="wrapper"]//div/h1/text()').get()
        ...
        # 提取时间
        date = response.xpath('///div[@class="article"]/p/span[1]/text()').get().strip()

        # 提取内容
        text_content = response.xpath('//div[@class="wp_articlecontent"]/p/span/text()').getall()
